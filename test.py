...
def diffMask(img1=None,img2=None,opt=None,dataset=None,args=None):
	...
	msk1 = msk1.cpu().detach().numpy()
	msk1 = (msk1 - np.min(msk1))/(np.max(msk1) - np.min(msk1))
	plt.imsave("{}/{}_{}_mask.jpg".format(save_masks,f,img1[:-6]),msk1,cmap="gray")
	plt.imsave("{}/{}_{}_ref.jpg".format(save_ref,f,img1[:-6]),resize(plt.imread("./data/{}/image/{}_0.jpg".format(opt.datamode,img1[:-6])),(128,128)))
...
def saveFullTranslation(image1=None,image2=None,opt=None,f=0):
    ...
    im3 = im2*(1-ms)
    im3[im3==0]=1; im_ress = im3*im88; im_ress = (im_ress - np.min(im_ress))/(np.max(im_ress) - np.min(im_ress)) if (np.min(im_ress) < 0 or np.min(im_ress) >1) else im_ress

    plt.imsave("./res_1.jpg",im_ress)
